    ...
    public:
        ...
        void imprime(NoTR* raiz){
            if(raiz != NULL){
                ...
                cout << "key: "<< raiz->key << " | prioridade: %d "<< raiz->prioridade;
                if (raiz->esquerdo) cout << " | filho esquerdo: " << raiz->esquerdo->key;
                if (raiz->direito) cout << " | filho direito: " << raiz->direito->key;
                cout << endl;
...
void fVO(int q, string instrucoes[], vector<string> palavras){
    ...
    while(i < q*2){
        ...
        //clock_t t = clock();    //conta o tempo de execucao
        ...
        //cout<< "Tempo de execucao para a instrucao " << inst << ": " << (float)t/CLOCKS_PER_SEC << " segundos." << endl;
    }
    //ts.imprime();
    return;
}
...
int main(){
    ...
    //t = clock() - t;
    //std::cout.precision(20);
    //cout<< "Tempo de execucao para a estrutura " << estrutura << ": " << (double)t/CLOCKS_PER_SEC << " segundos.";
